    ...
    try {
      const response = await fetch("http://localhost:8080/api/v1/calculo/impacto", {
        ...
        body: JSON.stringify({
          ...
          tipoCombustivel: "gasolina", // hardcoded para manter a API
      ...
      <div className="flex-1 space-y-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1.5">Tipo de veículo</label>
              <select
                value={tipoVeiculo}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-600 mb-1.5">Modelo específico</label>
            <input
              type="text"
        ...
        <div>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-semibold text-gray-900">Informações de uso</h3>
          </div>

          <div className="space-y-4 mb-4">
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1.5">Valor médio por pedágio</label>
              <div className="relative">
                <span className="absolute left-4 top-2.5 text-gray-500 text-sm">R$</span>
                <input
                  type="text"
                  value={valorMedio}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-600 mb-1.5">Pedágios por mês</label>
              <input
                type="number"
          ...
          <div className="bg-gray-50 text-gray-600 text-sm p-4 rounded-xl mt-6 border border-gray-200">
            <p className="font-medium text-gray-800 mb-1">Parâmetros de cálculo base:</p>
      ...
      <div className="flex-1 flex items-center justify-center min-h-[400px]">
        {resultado ? (
          <div className="w-full h-full bg-gray-50 rounded-2xl border border-gray-200 p-8 flex flex-col justify-center">
            <h4 className="text-xl font-bold text-gray-900 mb-2">Sua economia</h4>
            {/* Renderize os dados da API aqui. Exemplo genérico: */}
            <p className="text-gray-600 mb-6">Com base nas suas {resultado.totalPassagens || pedagiosPorMes} passagens mensais:</p>
            <div className="bg-white p-6 rounded-xl border border-emerald-100 shadow-sm mb-4">
              <div className="text-sm text-gray-500 mb-1">Economia estimada</div>
              <div className="text-4xl font-bold text-emerald-600">R$ {resultado.economiaEstimada || "120,50"}</div>
            </div>
            <div className="bg-white p-6 rounded-xl border border-emerald-100 shadow-sm">
              ...
              <div className="text-4xl font-bold text-emerald-600">{resultado.co2Evitado || "0.0"} kg</div>
            </div>
          </div>
        ) : (
          <div className="w-full h-full bg-gray-50/50 rounded-2xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center p-8 text-center">
            ...
            <h4 className="text-gray-900 font-semibold mb-1">Preencha os dados</h4>
            <p className="text-sm text-gray-500">Sua economia será calculada aqui</p>
          </div>
        )}
      </div>
    </div>
